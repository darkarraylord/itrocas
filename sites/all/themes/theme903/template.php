...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme903
 */

// Auto-rebuild the theme registry during theme development.
if ( theme_get_setting( 'theme903_clear_registry' ) ) {
	// Rebuild .info data.
 ...
 */
function theme903_html_head_alter( &$head_elements ) {
 ...
 */
function theme903_preprocess_search_block_form( &$vars ) {
 ...
 */
function theme903_preprocess( &$vars, $hook ) {
	$vars['theme903_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function theme903_preprocess_html( &$vars ) {
	$vars['doctype'] = _theme903_doctype();
	$vars['rdf'] = _theme903_rdf( $vars );
	...
	if ( !$vars['is_front'] ) {
		...
		if ( isset( $page_name ) ) {
			$vars['classes_array'][] = theme903_id_safe( 'page-' . $page_name );
		}

		$vars['classes_array'][] = theme903_id_safe( 'section-' . $section );
	...
	if ( ( $d['destination'] == 'blog' ) && ( theme_get_setting( 'theme903_blog_title' ) != "" ) ) {
		$vars['title'] = theme_get_setting( 'theme903_blog_title' );
		$vars['head_title'] = theme_get_setting( 'theme903_blog_title' );
	}
}
 ...
 */
function theme903_preprocess_page( &$vars ) {
 ...
 */
function theme903_preprocess_maintenance_page( &$vars ) {
	...
	drupal_add_css( drupal_get_path( 'theme', 'theme903' ) . '/css/maintenance-page.css' );
}
 ...
 */
function theme903_preprocess_node( &$vars ) {
	// Striping class
 ...
 */
function theme903_preprocess_block( &$vars, $hook ) {
 ...
 */
function theme903_menu_tree( $vars ) {
 ...
 */
function theme903_field__taxonomy_term_reference( $vars ) {
	$output = '';
 ...
 */
function theme903_breadcrumb( $vars ) {
	$breadcrumb = isset( $vars['breadcrumb'] ) ? $vars['breadcrumb'] : array();

	$condition = theme_get_setting( 'theme903_breadcrumb_hideonlyfront' ) ? count( $breadcrumb ) > 1
			: !empty( $breadcrumb );
	$separator = theme_get_setting( 'theme903_breadcrumb_separator' );

	if ( theme_get_setting( 'theme903_breadcrumb_showtitle' ) ) {
 ...
 * @return bool
 */
function theme903_tabs_float() {
	$float = (bool) theme_get_setting( 'theme903_tabs_float' );
	$float_node = (bool) theme_get_setting( 'theme903_tabs_node' );
...
function theme903_id_safe( $string ) {
	// Strip accents
 ...
 * @ingroup themeable
 */
function theme903_menu_link( array $vars ) {
	...
	$element['#attributes']['class'][] = theme903_id_safe( $element['#title'] );
 ...
 */
function theme903_preprocess_menu_local_task( &$vars ) {
 ...
 */
function theme903_menu_local_tasks( &$vars ) {
	$output = '';
 ...
 */
function theme903_preprocess_comment( &$variables ) {
